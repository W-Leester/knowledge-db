...
├── docs/                              ← 기획/설계 문서
│   ├── 01_요구사항정의서.md            ← 기능/비기능 요구사항 (결정 대기 항목 포함)
...
├── content/                           ← (폐기) 상위 Knowledge_DB 폴더를 직접 배포하는 것으로 설계 변경
├── tools/                             ← 보조 스크립트 (Phase 4 검색 인덱스 생성 등)
└── app/                               ← (예비) TWA/Capacitor 승격 시 사용

※ 앱의 실제 소스(index.html, assets/, sw.js 등)는 이 폴더가 아니라
   상위 `Knowledge_DB/` 루트에 위치한다 — 설계문서 3장 참조.
```
