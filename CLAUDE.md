...
- Android 브라우저의 강제 다크 반전은 `dark.css`의 `color-scheme` 선언으로 차단됨 — 건드리지 말 것. 라이트박스(`#lightbox img`)는 ID 셀렉터라 다크모드 매트 규칙을 이겨서 확대 보기의 배경이 어두워지는 버그가 있었음 — `dark.css`가 `!important`로 밝은 매트를 강제하므로 이 규칙도 유지.
