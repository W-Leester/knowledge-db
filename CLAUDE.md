...
**새 문서는 복제로 시작한다.** 백지에서 쓰지 말고 기존 `docs/*.html`을 복제해 시작한다 — 공통 셸(디자인 토큰·사이드바·hero·카드·callout·표·figure·코드블록·아코디언·라이트박스·진행바·모바일 드로어·반응형 `@media`)이 약 250줄로 전 문서가 동일하다. 교체하는 것은 본문(hero 카피·섹션·footer)과 hero 그라데이션 색뿐. **꼬리(라이트박스~`</html>`)는 줄 번호가 아니라 `<!-- Lightbox -->` 앵커로 추출할 것** — 원본 문서가 자라면 줄 번호 추출이 본문 한가운데를 잘라온다 (실제 사고: 원본이 90줄 커진 뒤 줄 번호로 뜬 문서에 스크립트·닫는 태그가 통째로 빠짐). 접합 후 `</html>`·`scroll-spy` 존재를 반드시 확인.
...
- **인라인 동적 도식(CSS 애니메이션 SVG)의 래퍼는 반드시 `<figure class="XX-anim">` 형태로** — dark.css가 `figure[class*="-anim"]` 패턴에 다크모드 밝은 매트를 깔아준다. 다른 이름을 쓰면 다크모드에서 글자가 배경에 묻힌다 (실사고: 래퍼 배경이 어두워지는데 내부 SVG는 라이트 전제).
- Android 브라우저의 강제 다크 반전 차단은 **2중**: `dark.css`의 `color-scheme` 선언 + 각 HTML head의 `<meta name="color-scheme" content="light dark">` (CSS만으로는 삼성 인터넷 "웹사이트 어둡게"가 이미지를 반전시킨 실사고 있음 — 새 문서에도 meta 필수, 셸 복제 시 자동 포함됨). 건드리지 말 것. 라이트박스(`#lightbox img`)는 ID 셀렉터라 다크모드 매트 규칙을 이겨서 확대 보기의 배경이 어두워지는 버그가 있었음 — `dark.css`가 `!important`로 밝은 매트를 강제하므로 이 규칙도 유지.

**SVG 저작.** 처음부터 모바일 가독 기준으로 그린다: viewBox 폭 ~900, 본문 글자 ≥13.5px. (다 그린 뒤 글자만 키우면 요소가 겹친다.) `font-family`는 SVG 안에 인라인. 밝은 배경 전제로 그린다 — 다크모드에서 `dark.css`가 `img[src*="img/"]`에 밝은 매트를 자동으로 깐다. **명도 대비 필수** (두 번 지적받은 실수 — 2026-08 전수 점검에서 954건 적발): 밝은 배경 위 회색 글씨는 `#666` 이상 진하게 — **`--text-muted`(`#8A8A8A`)·`#8A8478`을 SVG 텍스트에 그대로 쓰지 말 것**(화면 UI 토큰이지 도식용이 아니다). `#DC2626`을 `#FEE2E2` 위에 쓰는 조합(대비 3.95)도 미달이니 진한 변형을 쓴다. 흰 글씨를 얹는 막대·상자는 배경을 충분히 어둡게 — `#B5AFA5`·`#8A8A8A` 같은 중간 회색 위 흰 글씨 금지. 어두운 배경(`#0D1B2E`) 위 보조 글씨는 `#AAB9CF` 이상 밝게.
